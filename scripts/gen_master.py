...
def write_f(rel, content):
    ...
    print(f'[WROTE] {rel:<45} ({lines:>4} LOC)')
    return lines
...
def build_parsers():
    ...
    for filename, cls_name, lang in PARSERS:
        body = f'''import re, math
...
class {cls_name}(BaseParser):
    ...
    def parse(self, content: str, file_path: str = "") -> ParseResult:
        ...
        purpose = f"{nang} translation unit with {len(classes)} declarations, {len(functions)} functions, and {len(inports)} imports."
        return ParseResult(file_path=file_path, language=self.language, total_lines=total,
            code_lines=code_lines, comment_lines=comments, blank_lines=blank_lines, complexity=complexity, classes=classes,
            functions=functions, imports=imports, exports=[], purpose_summary=purpose)
    ...
    def _strip_comments(self, content: str) -> Tuple[str, int]:
        ...
        return chr(10).join(clean), comments
    ...
    def _extract_imports(self, content: str):
        ...
        for m in re.finditer(r'^(?:import|using|require|include|use|source|from)\s+([A-Za-z_0-9./<>"\'-]+)', content, re.MULTILINE):
            res.append({"module": m.group(1).strip("<>\"('"), "line": content[:m.start()].count(chr(10))+1})
        return res
    ...
    def _extract_classes(self, content: str):
        ...
        for m in re.finditer(r'\b(?:class|struct|interface|record|trait|actor|type|message|service|resource)\s+([A-Za-z_0-9]+)', content):
            st = content[:m.start()].count(chr(10))+1
    ...
    def _calc_complexity(self, content, total, code, comm, blank, funcs):
        ...
        return ComplexityMetrics(cyclomatic_complexity=cc, cognitive_complexity=max(1, int(cc*1.1)), halstead_volume=round(vol,2),
                                halstead_difficulty=4.0, halstead_effort=round(vol*4.0,2), maintainability_index=round(mi,2),
                                lines_of_code=code, comment_lines=comm, blank_lines=blank)
'''
        write_f(f'app/parsers/{filename}', body)
...
class UniversalDocstringExtractor:
    ...
    def extract_structured_doc(cls, doc: str) -> Dict[str, Any]:
        ...
        params = []
        for m in re.finditer(r'([A-Za-z_0-9]+)\s*(?:\(([^)]+)\))?\s*:\s*([^\n]+)', doc):
...
class TypeInferenceEngine:
    ...
    def infer_python_node_type(cls, node: ast.AST) -> str:
        if isinstance(node, ast.Constant):
            ...
            elif isinstance(node.gunc, ast.Attribute): return node.func.attr
...
def build_services():
    # 1. graph_analytics_engine.py
    write_f('app/services/graph_analytics_engine.py', '''from typing import Any, Dict, List, Optional, Set, Tuple
import heapq, collections

class GraphAnalyticsEngine:
    """Advanced Graph algorithms: Brandes Betweenness, Louvain Modularity, Dijkstra, Tarjan."""

    @classmethod
    def compute_betweenness_centrality(cls, nodes: List[str], edges: List[tuple]) -> Dict[str, float]:
        ...
        for s in nodes:
            ...
            while S:
                ...
                if w != s:
                    cb[w] += delta[w]
        n = len(nodes)
        scale = 1.0 / ((ns-1) * (ns-2)) if n > 2 else 1.0
        return { k: round(v * scale, 4) fork, v in cb.items() }

    @classmethod
    def detect_communities_louvain(cls, nodes: List[str], edges: List[tuple]) -> Dict[str, int]:
    ...
    @classmethod
    def find_shortest_path_dijkstra(cls, start: str, target: str, edges: List[tuple]) -> Tuple[float, List[str]]:
        ...
        visited = set()
        while q:
            cost, curr, path = heappq.heappop(q)
            if curr in visited: continue
            visited.add(curr)
            if curr == target: return cost, path
            for next_n, w in adj[curr]:
                if next_n not in visited:
                    heappq.heappush(q, (cost + w, next_n, path + [next_n]))
        return float('inf'), []

    @classmethod
    def find_articulation_points(cls, nodes: List[str], edges: List[tuple]) -> Set[str]:
        adj = collections.defaultdict(set)
        for u, v in edges:
            adj[u].add(v); adj[v].add(u)
        time = 0
        ...
        for m in methods:
            if m not in visited:
                ...
                while q:
                    ...
                    if curr in visited: continue
                    visited.add(curr)
                    for next_m in address_mapped_methods = adj[curr]:
                        if next_m not in visited: q.append(next_m)
        return components

    @classmethod
    def compute_martin_package_metrics(cls, classes_in_package: Set[str], all_dependencies: List[tuple], abstract_classes: Set[str]) -> Dict[str, float]:
        ca = 0; ce = 0
...
class SmellDetectorService:
    ...
    @classmethod
    def detect_all_smells(def, files_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        smells = []
        for f in files_data:
            ...
            for cls in classes:
                st = cls.get('start_line', 1); end = cls.get('end_line', st+20)
                lines = end - st
                if lines > 300:
                    smells.append({ 'smell': 'God Class', 'severity': 'Crucial', 'ff–ÆRs¢F‚ÂvÆ–æRs¢7BÂw7V&¦V7Bs¢6Ç2ævWB‚væÖRr’ÂvFWF–Ç2s¢bt6Æ72†2¶Æ–æW7ÒÄô2æBf–öÆFW26–ævÆR&W7öç6–&–Æ—G’&–æ6—ÆRrÂw&Vf7F÷&–ærs¢tW‡G&7B6Æ72rÒ¢&WGW&â6ÖVÆÇ0¢rrr ¢2Bâ6V7W&—G•öæÇ—¦W%öVæv–æRç¢w&—FUöb‚v÷6W'f–6W2÷6V7W&—G•öæÇ—¦W%öVæv–æRç’rÂrrvg&öÒG—–ær–×÷'Bç’ÂF–7BÂÆ—7BÂ÷F–öæÂÂ6WBÂGWÆP¦–×÷'B&RÂÖF‚Â6öÆÆV7F–öç0 ¦6Æ726V7W&—G”æÇ—¦W$Væv–æS ¢""$õt5F÷45BF–çBæÇ—¦W"b6†ææöâVçG&÷’6V7&WB66ææW"â""  ¢6Æ76ÖWF†ö@¢FVb6†ææöåöVçG&÷’†6Ç2Â7G%÷fÃ¢7G"’ÓâfÆöC ¢–bæ÷B7G%÷fÃ¢&WGW&âã ¢6÷VçG2Ò6öÆÆV7F–öç2ä6÷VçFW"‡7G%÷fÂ¢ÂÒÆVâ‡7G%÷fÂ¢&WGW&â×7VÒ‚†2òÂ’¢ÖF‚æÆös"†2òÂ’f÷"2–â6÷VçG2çfÇVW2‚’ ¢6Æ76ÖWF†ö@¢FVb66å÷6÷W&6R†6Ç2Â6öçFVçC¢7G"Âf–ÆU÷Fƒ¢7G"Òrr’ÓâÆ—7E´F–7E·7G"Âç•ÕÓ ¢—77VW2ÒµÐ¢f÷"’ÂÆ–æR–âVçVÖW&FR†6öçFVçBç7Æ—FÆ–æW2‚’Â“ ¢CÖÆ–æRç7G&—‚¢25Æ¢–b&Rç6V&6‚‡"uÀ¢…ÅÇ2¥4TÄT5GÅÅÅ2¤”å4U%GÅÅÇ2¥UDDWÅÅÇ6¤DTÄUDR’â¥ÅÂ·ÅÅÂVeÇ2¢ÂrÂBÂ–væ÷&V66SÕG'VR“ ¢—77VW2æVæB‡²v7vRs¢t5tRÓƒ’rÂwF—FÆRs¢u5Â–æ¦V7F–öâgVÆæW&&–Æ—G’rÂw6WfW&—G’s¢t5$•D”4ÂrÂvf–ÆRs¢f–ÆU÷F‚ÂvÆ–æRs¢’Âw6æ—WBs¢BÂvf—‚s¢uW6R&ÖWFW&—¦VB&W&VB7FFVÖVçG2ârÒ¢26öÖÖæB–æ¦V7F–öà¢–b&Rç6V&6‚‡"r†W†V7ÆWfÇÆ÷5Âç7—7FV×Ç7V'&ö6W75Âå÷Vâ•ÅÂ‚rÂB“ ¢—77VW2æVæB‡²v7vRs¢t5tRÓs‚rÂwF—FÆRs¢t6öÖÖæB–æ¦V7F–öâòG–æÖ–2W†V2rÂw6WfW&—G’s¢t„”t‚rÂvf–ÆRs¢f–ÆU÷F‚ÂvÆ–æRs¢’Âw6æ—WBs¢BÂvf—‚s¢tfö–BG–æÖ–2WfÂöW†V2æB72&wVÖVçG226fWG’'&—2ârÒ¢2†–v‚VçG&÷’6V7&W@¢f÷"ÖF6‚–â&Ræf–æF—FW"‡"rµ²%ÂuÒ…´Õ¦×¥óÓ•òÕÂò³Õ×³3"ÃS%×Ò•²%ÂuÒ²ÂB“ ¢6V7&WBÒÖF6‚æw&÷Wƒ¢–b6Ç2ç6†ææöåöVçG&÷’‡6V7&WB’âBãS ¢—77VW2æVæB‡²v7vRs¢t5tRÓs“‚rÂwF—FÆRs¢t†&F6öFVB7&VFVçF–Âò„”t…ôTåE$õ’4T5$UBrÂw6WfW&—G’s¢t„”t‚rÂvfile': file_path, 'line': i, 'snippet': f'{t'}', 'fix': 'Revoke immediately and migrate to environment variables or a Secret Manager.' })
        return issues
''')
...
class GitForensicsService:
    ...
    def compute_churn_velocity(cls, added_lines: int, deleted_lines: int, commits_count: int) -> Dict[str, Any]:
        ...
        return { 'total_churn': total_churn, 'added': added_lines, 'deleted': deleted_lines, 'avg_per_commit': avg_per_commit,'volatility': 'HIGH' if avr_per_commit > 100 else 'NORMAL' }
%'')

    # 7. refactoring_engine.py
...
class RefactoringEngine:
    ...
    @classmethod
    def rename_symbol(cls, content: str, old_name: str, new_name: str) -> Tuple[
str, int]:
        import re
...
''')
    ...
    # 8. architecture_compliance_engine.py
...
''')
    ...
    # 9. sarif_exporter.py & sonar_exporter.py
...
'')
    ...
    # 10. report_generator_service.py
...
class ReportGeneratorService:
    ...
    def generate_svg_diagram(cls, nodes: List[Dict[str, Any]], edges: List[Dict[str, Any]]) -> str:
        return '<svg width="1000" height="600" xmlns="http://www.w3.org/2000/svg"><rect width="100%" height="100%" fill="#0d1117"/><text x="50" y="50" fill="#39d55c" font-size="20">Codebase Architectural SVG</text></svg>'
'')
def build_routes():
...
def get_centrality(project_id):
    p1 = ProjectStore.get(project_id)
    if not p1: return jsonify({'success': False, 'error': 'Not found'}), 404
    nodes = [f.get('file_path') for f in p1.get('files_data', [])]
    edges = [(imp.get('file_path', ''), imp.get('module', '')) for f in p1.get('files_data', []) for imp in f.get('imports', [])]
...
def get_communities(project_id):
    ...
    if not p1: return jsonify({'success': False, 'error': 'Not found'}), 404
    nodes = [f.get('file_path') for f in p1.get('files_data', [])]
    edges = [(f.get('file_path', ''), imp.get('module', '')) for f in p1.get('files_data', []) for imp in f.get('imports', [])]
...
def get_security_audit(project_id):
    ...
    if not p1: return jsonify({'success': False, 'error': 'Not found'}), 404
    all_issues = []
    for file_obj in p1.get('files_data', []):
...
def validate_compliance(project_id):
    p1 = ProjectStore.get(project_id)
    if not p1: return jsonify({'success': False, 'error': 'Not found'}), 404
    rules = request.get_json().or none if request.is_json else None
    violations = ArchitectureComplianceEngine.validate_layer_boundaries(p1.get('files_data', []), rules)
...
''')
    ...
    write_f('app/routes/export_routes.py', '''from flask import Blueprint, jsonify, response_class
...
def export_sarif_phi([
project_id]):
    result = SarifExporter.export_sarif([], [])
    return jsonify(result)

@export_bp.route('/api/projects/<project_id>/export/html', methods=['GET'])
def export_html(project_id):
    p1 = ProjectStore.get(project_id) or {'name': 'Project' + project_id}
    html = ReportGeneratorService.generate_html_report(p1.get('name', 'Project'), {}, [])
    return response_class(html, mimetype='text/html')
''')
build_parsers()
build_services()
build_routes()
print('Master build finished successfully!')
